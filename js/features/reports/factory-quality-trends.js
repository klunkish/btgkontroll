...
(function(){
  ...
  function activeFactoryId(){
    try { return window.BTG_FACTORY_ADMIN_STATE?.selectedFactoryId || window.BTG_ACCESS_STATE?.profile?.factory_id || window.BTG_ACCESS_STATE?.factory?.id || window.BTG_ACCESS_STATE?.factory?.factory_id || localStorage.getItem('btg_selected_factory_id') || ''; }
    catch { return ''; }
  }
