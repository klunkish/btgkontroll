 ...
 * - Koppla silo till cement/material
 * - Registrera cementleverans som dagbokslik transaktion
...
(function(){
  ...
  async function getFactoryUserIds(fid){
    ...
    try {
      ...
      console.warn('Cement & Silor: kunde inte hämta fabriksanvändare, använder aktuell användare.', e);
      return [u.id];
    }
  }
  ...
  function recipeCementFallback(){
    const out = [];
    for (const r of state.recipes || []){
      for (const m of (Array.isArray(r.materials) ? r.materials : [])){
        if (String(m.type || '').toLowerCase().includes('cement') && String(m.name || '').trim()){
          out.push(normalizeLooseMaterial({ id:m.materialId || m.material_id || m.id || m.name, cloudId:m.cloudId || m.materialId || m.material_id || '', name:m.name, type:'Cement', factoryId:r.factoryId || '' }));
        }
      }
    }
  ...
  function mergeMaterials(list){
    ...
    for (const m of list.map(normalizeLooseMaterial).filter(Boolean)){
      const key = String(materialIdOf(m) || m.name).trim().toLowerCase();
      if (!key) continue;
      const prev = map.get(key);
      if (!prev || (!prev.type && m.type) || (!prev.cloudId && m.cloudId)) map.set(key, m);
    }
    return Array.from(map.values()).sort((a,b)=>String(a.name).localeCompare(String(b.name),'sv'));
  }
  function isCementMaterial(m){
    const t = String(m?.type || '').toLowerCase();
    const n = String(m?.name || '').toLowerCase();
    // Primärt typfältet, men fallback på namn eftersom äldre material ibland saknar type i Supabase-vyn.
    return t.includes('cement') || n.includes('cement') || /^cem\b/.test(n);
  }
  function allMaterialOptions(){
    return mergeMaterials([...(state.materials||[]), ...coreMaterialsFallback(), ...recipeCementFallback()]);
  }
  function materialOptions(){
    const all = allMaterialOptions();
    let cement = all.filter(isCementMaterial);
    // Om typfält saknas i gamla material, visa ändå alla material så silon går att koppla manuellt.
    if (!cement.length) cement = all;
    return cement.map(m => `<option value="${esc(materialIdOf(m) || m.name)}" data-name="${esc(m.name)}" data-type="${esc(m.type || '')}">${esc(m.name)}${m.type ? ' – ' + esc(m.type) : ''}</option>`).join('');
  }
  ...
  function ensureUI(){
    ...
    if (nav && !document.getElementById('btnCementSilos')){
      const btn = document.createElement('button');
      btn.className='tab-btn cement-nav-btn';
      btn.type='button';
      btn.id='btnCementSilos';
      btn.dataset.tab=TAB;
      btn.textContent='Cement & Silor';
    ...
    sec.innerHTML = `
      <div class="card"><div class="cement-silos-header"><div><h3>Cement & Silor</h3><p class="pill">Fabriksadmin: silonivåer beräknas från startsaldo + cementleveranser - receptförbrukning i satser.</p><div class="cement-small" id="cementSilosFactory"></div></div><div class="toolbar" style="justify-content:flex-end"><button class="btn secondary" id="btnCementRefresh" type="button">Uppdatera</button><button class="btn secondary" id="btnCementDelivery" type="button">Registrera cementleverans</button><button class="btn" id="btnCementAddSilo" type="button">Ny silo</button></div></div></div>
    ...
    document.getElementById('btnCementRefresh')?.addEventListener('click', loadAndRender);
    document.getElementById('btnCementAddSilo')?.addEventListener('click', () => openSiloDialog());
    document.getElementById('btnCementDelivery')?.addEventListener('click', () => openDeliveryDialog());
    ...
    card.innerHTML = `
      <div class="cement-dashboard-head">
        ...
        <div class="toolbar" style="justify-content:flex-end">
          <button class="btn secondary" id="btnCementDashboardRefresh" type="button">Uppdatera</button>
          <button class="btn" id="btnOpenCementSilosFromDashboard" type="button">Öppna Cement & Silor</button>
        </div>
      </div>
  ...
  async function loadData(){
    state.error = '';
    const s = db(), u = currentUser();
    if (!s || !u) { state.error = 'Logga in mot Supabase för att använda Cement & Silor.'; return; }
    ...
    const firstErr = [silosRes,txRes,matRes,recRes,batRes].find(r => r?.error)?.error;
    ...
    state.materials = mergeMaterials([...(state.materials||[]), ...coreMaterialsFallback(), ...recipeCementFallback()]).filter(x=>belongsSoft(x,fid));
    state.loadedAt = new Date();
    analyzeAll();
  }
  async function loadAndRender(){ try { await loadData(); } catch(e){ console.error(e); state.error = 'Kunde inte ladda Cement & Silor. Har du kört SQL-filen 19_5_31_cement_silos.sql i Supabase? ' + (e.message || ''); } render(); }
  ...
  function renderDashboardSilos(totalCap, totalNow, totalIn, totalOut, pct){
    ...
    if (!a.length){
      grid.innerHTML = `<div class="cement-empty"><b>Inga silos skapade ännu.</b><br>Klicka på Öppna Cement & Silor för att skapa första silon.</div>`;
      return;
    }
  ...
  function renderSiloGrid(){
    ...
    const a = state.analysis || [];
    if (!a.length){ el.innerHTML = `<div class="cement-empty"><b>Inga silos ännu.</b><br>Skapa en silo och koppla den till ett cement/material för att börja följa lager i kg och procent.</div>`; return; }
  ...
  function openSiloDialog(silo){
    ...
    sel.innerHTML = `<option value="">— välj cement/material —</option>${materialOptions()}`;
  ...
  async function saveDelivery(e){
    ...
    try { await s.from('diary_entries').insert([{ user_id:u.id, entry_date:date, note:`Cementleverans: ${silo.name}, ${silo.materialName}, ${kg} kg${note ? ' – ' + note : ''}`, imports:[], app_data:{ type:'cement_delivery', siloId:silo.id, siloName:silo.name, materialName:silo.materialName, amountKg:kg, note, factoryId:activeFactoryId() || null } }]); } catch(_){}
  ...
  window.BTG_CEMENT_SILOS = { open: openTab, refresh: loadAndRender, state };
})();
