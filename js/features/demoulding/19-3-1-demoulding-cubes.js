...
(function(){
  ...
  async function refreshDemouldingModule(){
    // 3.0.3: Avformningskuber ska inte trigga full molnladdning.
    // Full loadCloudCoreData sätter global laddindikator gul/grön och kan ge blink.
    // Här laddar vi bara avformningshistoriken och renderar sidan.
  ...
  async function openDemouldingPage(){
    ...
    const sec=document.getElementById('tab-avformningskuber');
    if(sec){
      sec.hidden=false;
    ...
    try{
      document.querySelectorAll('.tab-btn[data-tab], .main-nav [data-tab]').forEach(btn=>{
        btn.setAttribute('aria-current', btn.dataset.tab==='avformningskuber' ? 'page' : 'false');
        btn.classList.toggle('active', btn.dataset.tab==='avformningskuber');
      });
    }catch(_){}
    try{ if(typeof tabs!=='undefined') tabs.avformningskuber=sec; }catch(_){}
    await refreshDemouldingModule();
    try{ document.body.classList.remove('sidebar-open'); }catch(_){}
  }

  window.BTG_DEMOULDING = {
    open: openDemouldingPage,
    refresh: refreshDemouldingModule,
    renderToday: renderDemouldingToday,
    renderHistory: renderDemouldingHistory
  };
