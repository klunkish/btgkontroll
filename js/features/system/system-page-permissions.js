...
(function(){
  ...
  const PAGE_DEFS=[
    ...
    {key:'prefab',label:'Prefabkö',hint:'Prefabbeställningar, klarhantering och temperatursensorer',match:['prefab','prefabkö','prefabko']},
    {key:'orders',label:'Kunder & beställningar',hint:'Kunder, order och beställningar',match:['kund','beställ','order']},
    {key:'cubes',label:'Provningar',hint:'Provtagningar, kuber och hållfasthetsresultat',match:['provkub','provning','kuber']},
    {key:'demoulding',label:'Avformningskuber',hint:'Avformning och tidiga hållfastheter',match:['avform']},
    {key:'evaluation',label:'Utvärdering',hint:'EN206-utvärdering, statistik och kravuppföljning',match:['utvärdering','utvardering']},
    {key:'control',label:'Kontroll',hint:'Kontrollutrustning, kalibrering och kontroller',match:['kontroll']},
    {key:'materials',label:'Material',hint:'Materiallista, cement, ballast och tillsatsmedel',match:['material']},
    {key:'temperature_tables',label:'Bord & Temperatur',hint:'Bord, sensorer och temperaturkurvor',match:['bord','temperatur','sensor','termo']},
    {key:'diary',label:'Dagbok',hint:'Dagbok, leveranser och händelser',match:['dagbok']},
    {key:'cement_silos',label:'Material & Silor',hint:'Silonivåer, materiallager och förbrukning',match:['cement','silor','silo']},
    ...
    {key:'documents',label:'Offert / Faktura',hint:'Offert-, faktura- och orderdokument',match:['offert','faktura','dokument']},
    {key:'environment',label:'Miljö',hint:'Miljövarudeklarationer, EPD och receptrapporter',match:['miljö','miljo','klimat','epd']},
  ...
  function ensureDialog(){
    ...
    dlg.innerHTML=`<div class="sc-perm-inner">
      ...
      <div class="sc-perm-add-user">
        <div>
          <b>Lägg till användare i vald fabrik</b>
          <span>Om en ny testanvändare inte syns i listan kan du lägga till den via e-post här.</span>
        </div>
        <input id="scPermAddEmail" type="email" placeholder="anvandare@foretag.se">
        <select id="scPermAddRole">
          <option value="viewer">viewer</option>
          <option value="user">user</option>
          <option value="factory_admin">factory_admin</option>
          <option value="admin">admin</option>
        </select>
        <button class="btn secondary" type="button" id="scPermAddUser">Lägg till</button>
      </div>
    ...
    $('#scPermAddUser',dlg)?.addEventListener('click',async()=>{try{await addUserToSelectedFactory();}catch(err){alert('Kunde inte lägga till användare: '+(err.message||err));}});
  ...
  async function addUserToSelectedFactory(){
    const f=state.selectedFactory;
    const email=String($('#scPermAddEmail')?.value||'').trim().toLowerCase();
    const role=String($('#scPermAddRole')?.value||'viewer').trim();
    if(!f) return alert('Välj fabrik först.');
    if(!email || !email.includes('@')) return alert('Ange användarens e-postadress.');
    const data=await rpc('btg_systemcenter_add_user_to_factory',{p_email:email,p_factory_id:String(f),p_role_code:role});
    await loadFactoriesUsers();
    const user=(state.users||[]).find(u=>String(u.email||u.user_email||'').toLowerCase()===email || String(u.user_id||u.id||'')===String(data?.user_id||''));
    if(user) state.selectedUser=userId(user);
    $('#scPermAddEmail') && ($('#scPermAddEmail').value='');
    await refreshDialog(false);
    alert('Användaren är kopplad till fabriken.');
  }
  ...
  async function applyCurrentPermissions(){
    if(window.BTG_MENU_REGISTRY?.rebuild){ try{ await window.BTG_MENU_REGISTRY.rebuild(); return; }catch(_){} }
    ...
    buttons.forEach(btn=>{
      ...
      btn.dataset.btgPageKey = btn.dataset.btgPageKey || p.key;
      btn.dataset.btgPermissionMode = m;
      btn.dataset.btgLocked = m==='locked' ? '1' : '0';
