...
(function(){
  ...
  function init(){
    ensureUI();
    refreshAccess();
    [250,800,1600,3000].forEach(ms=>setTimeout(()=>{ensureUI(); refreshAccess(); enforceFactoryLock();},ms));
    try{ window.sb?.auth?.onAuthStateChange?.(()=>setTimeout(()=>{refreshAccess(); ensureUI(); enforceFactoryLock();},500)); }catch(_){ }
