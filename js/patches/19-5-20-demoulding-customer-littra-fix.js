(function(){
  if (window.__BTG_V19520_DEMOULDING_CUSTOMER_LITTRA_FIX) return;
  window.__BTG_V19520_DEMOULDING_CUSTOMER_LITTRA_FIX = true;
  ...
  function numText(v, d){ const n = Number(v); return Number.isFinite(n) ? n.toFixed(d) : '—'; }
  ...
  function normalizeBatch(row){
    ...
    return {
      ...
      requesterName: app.requesterName || app.requester_name || '',
  ...
  function tableNo(b){ const app = b?.app_data || {}; const raw = b?.tableNo || b?.table_no || app.tableNo || app.table_no || app.bord || app.bordsnr || ''; return (window.sanitizeDemouldingTableNo ? window.sanitizeDemouldingTableNo(raw) : raw); }
  function findRecipeName(b){
    if (b?.recipeName) return b.recipeName;
    const rid = b?.recipeCloudId || b?.recipeId;
    const rec = (Array.isArray(window.RECIPES) ? window.RECIPES : (typeof RECIPES !== 'undefined' ? RECIPES : [])).find(r => String(r.cloudId || r.id) === String(rid) || String(r.id) === String(rid));
    return rec?.name || '—';
  }
  function findStrength(b){
    if (b?.strength) return b.strength;
    const rid = b?.recipeCloudId || b?.recipeId;
    const recs = Array.isArray(window.RECIPES) ? window.RECIPES : (typeof RECIPES !== 'undefined' ? RECIPES : []);
    const rec = recs.find(r => String(r.cloudId || r.id) === String(rid) || String(r.id) === String(rid));
    return rec?.strength || '';
  }
  function uniq(rows){
    const seen = new Set();
  ...
  async function factoryUserIds(){
    ...
    try{
      ...
      console.warn('Avformning v19.5.20: kunde inte läsa fabriksanvändare:', err);
      return u?.id ? [u.id] : [];
    }
  }
  async function loadBatches(){
    ...
    const globals = Array.isArray(window.BATCHES) ? window.BATCHES : (typeof BATCHES !== 'undefined' && Array.isArray(BATCHES) ? BATCHES : []);
    let rows = [];
    if (s && ids.length){
      try{
        const { data, error } = await s.from('batches')
          .select('id,recipe_id,amount_m3,datetime,gwp,app_data,created_at,user_id')
          .in('user_id', ids)
          .order('datetime', { ascending:false })
          .limit(1000);
        if (error) throw error;
        rows = (data || []).map(normalizeBatch);
      }catch(err){
        lastError = err?.message || String(err || 'Kunde inte läsa satser');
        console.warn('Avformning v19.5.20: batches:', err);
      }
    }
    return uniq([...globals.map(normalizeBatch), ...rows]);
  }
  async function loadHistory(){
    const s = db();
    const u = user();
    if (!s || !u){ cachedHistory = []; return []; }
    const ids = await factoryUserIds();
    try{
      ...
      cachedHistory = (data || []).map(r => ({
        ...
        drawingNo: r.app_data?.drawingNo || r.app_data?.drawing_no || '',
      ...
      console.warn('Avformning v19.5.20: historik:', err);
      cachedHistory = [];
    }
    return cachedHistory;
  }
  function historyFor(b){ const id = batchCloudId(b); return cachedHistory.find(h => id && String(h.batchCloudId) === id) || null; }
  function patchHeader(){
    const tr = document.querySelector('#demouldingTodayTable thead tr');
    if (!tr) return;
    if (!tr.querySelector('[data-v19520-littra]') && !Array.from(tr.children || []).some(th => /littra|ritning/i.test(th.textContent || ''))){
      const th = document.createElement('th');
      th.textContent = 'Littra/ritning';
      th.setAttribute('data-v19520-littra','1');
      tr.insertBefore(th, tr.children[3] || null);
    }
    if (!tr.querySelector('[data-v19523-table-no]') && !Array.from(tr.children || []).some(th => /^(bord|bordsnr|bord nr)$/i.test((th.textContent || '').trim()))){
      const th = document.createElement('th');
      th.textContent = 'Bord';
      th.setAttribute('data-v19523-table-no','1');
      const littraTh = tr.querySelector('[data-v19520-littra]');
      tr.insertBefore(th, littraTh ? littraTh.nextSibling : (tr.children[4] || null));
    }
  }
  function setStatus(all, rows){
    ...
    if (status) status.textContent = `Avformning v19.5.25 • prefab-satser: ${(all||[]).filter(isPrefabBatch).length} • valt datum: ${rows.length}` + (lastError ? ` • ${lastError}` : '');
  }
  async function save(batch, row){
    const s = db();
    const u = user();
    if (!s || !u){ alert('Supabase inte tillgänglig eller användare saknas. Ladda om appen.'); return; }
    const box = row?.querySelector('.demoulding-input-row');
    ...
    const app = batch.app_data || {};
    const recName = findRecipeName(batch);
    const strength = findStrength(batch);
    const customerName = String(batch.customerName || app.customerName || app.customer_name || 'Prefab' || '').trim();
    const littra = drawingNo(batch);
    const appData = {
      ...
      customerId: batch.customerId || app.customerId || null,
      customerName,
      drawingNo: littra,
      tableNo: tableNo(batch),
      prefabRequestId: batch.prefabRequestId || app.prefabRequestId || app.prefab_request_id || null,
      factoryId: batch.prefabFactoryId || app.prefabFactoryId || app.factory_id || activeFactoryId(),
    ...
    const payload = {
      user_id: u.id,
      batch_cloud_id: appData.batchCloudId || null,
      batch_id: appData.batchId || '',
      batch_datetime: appData.batchDateTime || null,
      cast_date: appData.castDate || selectedDate(),
      demould_date: appData.demouldDate,
      recipe_id: appData.recipeId || null,
      recipe_name: appData.recipeName || '',
      strength: appData.strength || '',
      production_type: 'prefab',
      customer_id: appData.customerId || null,
      customer_name: appData.customerName || 'Prefab',
      prefab_request_id: appData.prefabRequestId || null,
      factory_id: appData.factoryId || null,
      amount_m3: Number(appData.amount) || null,
      mpa,
    ...
    const { error } = await s.from('demoulding_cubes').insert([payload]);
    if (error){ alert('Kunde inte spara avformningskub: ' + (error.message || error)); return; }
    await render();
  }
  function knownDates(all){ return Array.from(new Set((all||[]).filter(isPrefabBatch).map(batchDate).filter(Boolean))).sort().reverse().slice(0,10); }
  async function render(){
    ...
    try{
      patchHeader();
      const [all] = await Promise.all([loadBatches(), loadHistory()]);
      const rows = (all || []).filter(isPrefabBatch).filter(b => batchDate(b) === selectedDate()).sort((a,b) => String(a.dateTime || '').localeCompare(String(b.dateTime || '')));
      setStatus(all, rows);
      if (!user()){
      ...
      if (!rows.length){
        const dates = knownDates(all);
        tbody.innerHTML = `<tr><td colspan="10">Inga prefabgjutningar hittades för valt gjutdatum. ${dates.length ? 'Datum med prefab-satser: ' + dates.map(d => `<button class="btn secondary" type="button" data-v19520-demould-date="${esc(d)}" style="margin:.15rem">${esc(fmtDateOnly(d))}</button>`).join(' ') : 'Inga prefab-satser hittades i aktiv fabrik.'}</td></tr>`;
        tbody.querySelectorAll('[data-v19520-demould-date]').forEach(btn => btn.addEventListener('click', () => {
          const input = document.getElementById('demouldingCastDate');
          if (input) input.value = btn.getAttribute('data-v19520-demould-date') || '';
          render();
        }));
        return;
      }
      tbody.innerHTML = rows.map((b, idx) => {
        const done = historyFor(b);
        const recName = findRecipeName(b);
        const strength = findStrength(b);
        const customer = b.customerName || b.app_data?.customerName || b.app_data?.customer_name || 'Prefab';
        ...
        return `<tr${done ? ' class="demoulding-done-row"' : ''}>
          ...
          <td>Prefab</td>
          <td>${esc(customer)}</td>
          <td>${numText(b.amount, 2)} m³</td>
          <td>${done ? `<span class="pill ok">Sparad ${esc(fmtDateOnly(done.demouldDate))}</span><span class="demoulding-small">${numText(done.mpa,1)} MPa • ${esc(done.employeeNo)}</span>` : `
            <div class="demoulding-input-row">
              ...
              <button class="btn" type="button" data-v19520-save="${idx}">Klar</button>
            </div>`}</td>
          <td>${done ? '<span class="pill ok">Klar</span>' : '<span class="pill warn">Ej klar</span>'}</td>
        </tr>`;
      }).join('');
      tbody.querySelectorAll('[data-v19520-save]').forEach(btn => btn.addEventListener('click', () => save(rows[Number(btn.getAttribute('data-v19520-save'))], btn.closest('tr'))));
    } finally {
      loading = false;
    }
  }
  function bind(){
    ...
    document.addEventListener('click', e => {
      if (e.target?.id === 'btnRefreshDemoulding' || e.target?.dataset?.tab === 'avformningskuber') setTimeout(render, 120);
    }, true);
  }
  const oldRefresh = window.btgRefreshDemouldingV19515;
  window.btgRefreshDemouldingV19515 = render;
  window.btgRefreshDemouldingV19520 = render;
  function init(){ bind(); if (document.getElementById('tab-avformningskuber')) setTimeout(render, 400); }
