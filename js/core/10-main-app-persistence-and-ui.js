/***********************
  ...
  window.BTG_DIRECT_CORE_OPEN = async function(name){
    const tabMap={production:'tillverkning',orders:'bestallningar',cubes:'provningar',evaluation:'utvardering',control:'kontroll',materials:'material',diary:'dagbok',dashboard:'dashboard'};
    name = tabMap[name] || name;
    ...
    }catch(err){ console.warn('Kunde inte ladda om molndata vid direkt fliköppning:', err); }
    try{
      if (name==='dashboard') renderDashboard();
      if (name==='tillverkning') { renderRecipes(); renderRecipeSelects(); renderBatchForm(); renderBatches(); }
      if (name==='provningar') { renderCubeForm(); renderCubes(); renderAirForm(); renderAir(); }
      ...
      if (name==='material') { renderMaterials(); }
      if (name==='dagbok') { renderDiaryInit(); }
    }catch(err){ console.warn('Kunde inte rendera direkt öppnad flik:', err); }
...
try{ window.BTG_OPEN_CORE_TAB = async function(name){ return await setTab(name); }; }catch(_e){}
